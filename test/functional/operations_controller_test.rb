...
class OperationsControllerTest < ActionController::TestCase
  ...
  def test_should_post_create_for_user_negative_timezone
    ...
    assert_difference 'Operation.count' do
      post :create, :project_id => 1, :operation => {:description => "New operation description",
          ...
          :amount => 1000,
          :category_id => 1,
          :operation_date => '2017-04-20'}, :operation_time => '11:11'
      assert_response :redirect
    end
  ...
  def test_filter_by_ids
    @request.session[:user_id] = 1
    ids = [3,2]
    get :index, :project_id => 1, :set_filter => 1, "f"=>["ids", ""], "op"=>{"ids"=>"="}, "v"=>{"ids"=>[ids.join(",")]}
    assert_response :success
    assert_equal ids.sort, assigns(:operations).map(&:id).sort
  ...
  def test_should_get_index_with_saved_query
    ...
    @request.session[:user_id] = 1
    get :index, :query_id => operation_query.id, :project_id => Project.find(1).identifier
    assert_response :success
    assert_template :index
    assert_not_nil assigns(:operations)
  end
