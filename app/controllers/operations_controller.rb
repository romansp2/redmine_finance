...
class OperationsController < ApplicationController
  ...
  def create
    ...
    @operation.operation_date = calculated_date(params) if [params[:operation][:operation_date], params[:operation_time]].all?{ |p| p.present? }
  ...
  def calculated_date(params)
    Time.use_zone(User.current.time_zone) do
      Time.zone.parse([params[:operation][:operation_date], params[:operation_time]].join(' '))
    end
  end
end
